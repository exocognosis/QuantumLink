...
After successful activated network application, the daemon opens the configured TUN device, initializes a `qlink-core` packet tunnel core, and publishes `dataPlane` status. This confirms local packet I/O initialization, not live peer reachability. Until the SteamOS daemon is wired to the live mesh transport, `dataPlane.state` remains `starting`, `transportReady` remains false, and `qlinkctl doctor` reports that boundary explicitly.
...
The SteamOS silo contains the daemon, CLI, Linux TUN/network helpers, systemd unit, installer assets, and game profile helpers. The shared protocol core remains in `qlink-core`. Production readiness still requires live mesh-transport wiring from the SteamOS packet pump, Deck-host data-plane validation, hardened nftables rollback, public rendezvous/relay hardening, signed release packaging, and broader game compatibility testing.
