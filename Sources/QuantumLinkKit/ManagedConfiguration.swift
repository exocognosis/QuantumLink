...
///   - `dytallixAllowedRpcEndpoints`: array of strings
///
...
public enum ManagedConfigurationLoader {
  ...
  public static func apply(
    ...
    var mtu = base.mtu
    var meshTrustPolicy = base.meshTrustPolicy
    var discoveryIdentityMode = base.discoveryIdentityMode
    var dytallixEndpoint = base.dytallixIdentity?.endpoint
    var dytallixContractAddress = base.dytallixIdentity?.contractAddress
    var dytallixNetworkID = base.dytallixIdentity?.networkID
    var dytallixChainID = base.dytallixIdentity?.chainID
    var dytallixAllowedRPCEndpoints = base.dytallixIdentity?.allowedRPCEndpoints ?? []
    var publishWalletAddress = base.dytallixIdentity?.publishWalletAddress ?? false
    ...
    for (key, value) in managed {
      ...
      case "meshTrustPolicy":
        if let stringValue = value as? String,
          let parsed = MeshTrustPolicy(rawValue: stringValue)
        {
          meshTrustPolicy = parsed
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "discoveryIdentityMode":
        if let stringValue = value as? String,
          let parsed = DiscoveryIdentityMode(rawValue: stringValue)
        {
          discoveryIdentityMode = parsed
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "dytallixEndpoint":
        if let stringValue = value as? String, !stringValue.isEmpty {
          dytallixEndpoint = stringValue
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "dytallixContractAddress":
        if let stringValue = value as? String, !stringValue.isEmpty {
          dytallixContractAddress = stringValue
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "dytallixNetworkId":
        if let stringValue = value as? String, !stringValue.isEmpty {
          dytallixNetworkID = stringValue
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "dytallixChainId":
        if let stringValue = value as? String, !stringValue.isEmpty {
          dytallixChainID = stringValue
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "dytallixAllowedRpcEndpoints":
        if let arrayValue = value as? [String] {
          dytallixAllowedRPCEndpoints = arrayValue
          applied.append(key)
        } else {
          rejected.append(key)
        }
      case "publishWalletAddress":
        if let boolValue = value as? Bool {
          publishWalletAddress = boolValue
          applied.append(key)
        } else {
          rejected.append(key)
        }
    ...
    let dytallixIdentity: DytallixIdentityConfiguration? =
      if let dytallixEndpoint, let dytallixContractAddress {
        DytallixIdentityConfiguration(
          endpoint: dytallixEndpoint,
          contractAddress: dytallixContractAddress,
          publishWalletAddress: publishWalletAddress,
          networkID: dytallixNetworkID,
          chainID: dytallixChainID,
          allowedRPCEndpoints: dytallixAllowedRPCEndpoints
        )
      } else {
        nil
      }
