...
- Rust protocol core with an ML-KEM handshake, device signatures, signed peer records, route policy, replay protection, rendezvous, PQC relay fallback through configured or signed `quantum_link_relay` candidates, STUN candidate gathering, and feature-gated TURN relay-candidate gathering.
