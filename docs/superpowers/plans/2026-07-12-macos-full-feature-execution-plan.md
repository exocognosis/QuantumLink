...
- **Next up (both need the same file):** reconcile `mesh_transport.rs` + `mesh_connection.rs` from silo so the ported carrier + PQC-session + identity modules actually drive the live mesh dial path (they currently land + unit-test in isolation but are not yet wired into mesh traffic). Then: Swift enrollment/discovery-identity UX; `qlinkctl` dytallix subcommands; wire the deployed testnet contract address (deploy in progress — user-run).
