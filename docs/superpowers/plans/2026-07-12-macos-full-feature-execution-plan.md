...
- **(b) Modular Swift identity module DONE** (commit `aa9b5a1`): self-contained `DytallixIdentity.swift` (DiscoveryIdentityMode, MeshTrustPolicy, wire config + composer seam), `DytallixEnrollmentSettings`, `DiscoveryIdentityPresentation` (decoupled), `DytallixEnrollmentCommandOutput`; `MeshTransportConfiguration` gains optional `meshTrustPolicy`/`dytallixIdentity` (1:1 with the FFI). Chose a modular seam over the silo's `TunnelConfiguration`-coupled approach (didn't drag in the 725-line-diverged `Models.swift` or the dev-quic-disabling `TunnelTransport` rewrite). **Verified without Xcode:** `swift build` compiles; a scratch encoder confirms the Swift API emits the exact enforcement JSON; Rust test `swift_encoded_public_mesh_config_enforces_registry` parses that literal → `PublicRequired` + deployed contract (round-trip proven).
- **(d) Phase 2 — TURN relay client DONE** (commit `a9496aa`): standard TURN (RFC 5766/8656) client (`turn.rs`) — `TurnClient::allocate` with unauthenticated + long-term-credential (401→MESSAGE-INTEGRITY) flow, `gather_relay_candidate(s)` producing `CandidateType::Relay` into the existing RFC-8445 nomination model, `DevTurnServer` + 8 tests, `md-5` dep. Completes the connection-type matrix (LAN host / direct-STUN-reflexive / relay) with a standard-infra (coturn) relay source alongside the native relay. qlink-core suite **193 pass**. Remaining Phase 2 polish: expose `turnServers` in the mesh config + wire `gather_relay_candidates` into mesh self-candidate publishing (the mesh doesn't gather STUN either yet — a shared follow-up), and a `qlinkctl --turn` flag.
- **Still pending full Xcode:** behavioral Swift tests (`swift test` needs `XCTest`), app-UI wiring in `QuantumLinkApp.swift`, and `.xcframework` packaging (`xcodebuild -create-xcframework`). Copied enrollment files keep silo 2-space indent (no formatter gate). Then (e) dev-quic test-timeout polish; Phase 5/6 (Apple signing + hardware validation). NB: prior `qlinkctl` WIP preserved in `git stash`.
