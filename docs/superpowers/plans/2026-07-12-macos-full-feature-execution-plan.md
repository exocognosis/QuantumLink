...
- **Next up:** (a) construct the mesh runtime's `DytallixIdentityRegistry` from `config/dytallix-testnet.json` + set public meshes to `PublicRequired` (currently dev default is `None`/`DevelopmentOptional`); (b) Swift enrollment + discovery-identity (Off/Verified/Public-Wallet) UX + `DytallixEnrollmentSettings`; (c) Swift-side build/tests (needs the Rust xcframework rebuilt for SHAKE256); (d) Phase 2 ICE/STUN/TURN completion; (e) dev-quic test-timeout polish. NB: a prior local `qlinkctl` WIP (packet round-trip helper) is preserved in `git stash` (superseded by the silo CLI).
