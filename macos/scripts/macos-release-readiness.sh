...
xcconfig_value() {
  ...
  ' config/QuantumLink.common.xcconfig
}

log "Required macOS project files"
require_file project.yml
require_file config/QuantumLink.common.xcconfig
require_file config/QuantumLink.unsigned.xcconfig
require_file config/QuantumLink.developer-id.template.xcconfig
require_file Info/QuantumLink.Info.plist
require_file Info/QuantumLinkTunnel.Info.plist
require_file entitlements/QuantumLink.entitlements
require_file entitlements/QuantumLinkTunnel.entitlements
require_file scripts/package-macos.sh
require_file scripts/generate-xcode-project.sh

log "Plist syntax"
for plist in \
  Info/QuantumLink.Info.plist \
  Info/QuantumLinkTunnel.Info.plist \
  entitlements/QuantumLink.entitlements \
  entitlements/QuantumLinkTunnel.entitlements
do
...
for entitlements in entitlements/QuantumLink.entitlements entitlements/QuantumLinkTunnel.entitlements; do
...
require_contains project.yml "CODE_SIGN_ENTITLEMENTS: entitlements/QuantumLink.entitlements" "app target uses app entitlements"
require_contains project.yml "CODE_SIGN_ENTITLEMENTS: entitlements/QuantumLinkTunnel.entitlements" "tunnel target uses tunnel entitlements"
require_contains project.yml "INFOPLIST_FILE: Info/QuantumLink.Info.plist" "app target uses app Info.plist"
require_contains project.yml "INFOPLIST_FILE: Info/QuantumLinkTunnel.Info.plist" "tunnel target uses tunnel Info.plist"
require_contains project.yml "PRODUCT_BUNDLE_IDENTIFIER: \$(QLINK_APP_BUNDLE_ID)" "app target uses QLINK_APP_BUNDLE_ID"
require_contains project.yml "PRODUCT_BUNDLE_IDENTIFIER: \$(QLINK_TUNNEL_BUNDLE_ID)" "tunnel target uses QLINK_TUNNEL_BUNDLE_ID"
require_contains project.yml "PROVISIONING_PROFILE_SPECIFIER: \$(QLINK_APP_PROVISIONING_PROFILE_SPECIFIER)" "app target uses app provisioning profile specifier"
require_contains project.yml "PROVISIONING_PROFILE_SPECIFIER: \$(QLINK_TUNNEL_PROVISIONING_PROFILE_SPECIFIER)" "tunnel target uses tunnel provisioning profile specifier"
require_contains Info/QuantumLinkTunnel.Info.plist "com.apple.networkextension.packet-tunnel" "tunnel Info.plist declares packet tunnel extension point"
