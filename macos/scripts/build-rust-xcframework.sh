...
CARGO_TARGET_ROOT="${QLINK_CARGO_TARGET_DIR:-$REPO_ROOT/target}"
...
EOF
  ...
  (cd "$REPO_ROOT" && CARGO_TARGET_DIR="$CARGO_TARGET_ROOT" cargo build -p qlink-core --release --target "$target")
