...
/// encrypts the local `peers.json` cache. Mirrors `DeviceKeypairStore`
/// in shape and lifecycle, but stores a 32-byte ChaCha20-Poly1305
/// key rather than an ML-DSA seed.
///
...
/// machines or in lost-laptop scenarios. The Rust core's v2 envelope
/// wraps the cache with ChaCha20-Poly1305 AEAD when supplied a
/// 32-byte key; this class is the Swift host's side of that
/// contract.
///
...
public final class PeerStoreKey {
    ...
    /// invariant that lets a previously-encrypted `peers.json`
    ...
    /// will mint a fresh one — which means any pre-existing v2
